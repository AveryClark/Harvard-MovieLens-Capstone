...
Now we'll split up the data into our training set and 
our validation set.

The training set will be 90% of the MovieLens data and 
the validation set will be 10%.

We'll set a random seed so the results can be reproduced 
by anyone else running this code.

```{r Analysis 02, include=TRUE}
...
As we can see, the validation set has 999,999 
entries and the same 6 columns.

Thankfully, this data is already pretty clean. I won't 
have to go through a lot of effort looking for ways 
to fix errors or NAs from bad data entry or missing data.

Take note that the entries in the validation set ARE 
NOT in the training set (edx set), and vice versa. 
No entries are shared between the sets.

We'll train our model/algorithm on the training set 
and test its accuracy on the validation set.


Let's see how many unique users and movies are in 
the training dataset.

```{r Analysis 04, include=TRUE}
edx %>%
...
To explain this step by step, we subtract the predicted ratings from the true ratings,
...
If we build a "naive" prediction model, predicting the 
ratings of movies in the validation set using only the 
average of ratings in the training set, we get a RMSE of 1.061.


```{r Results 01 c, include=TRUE}
onlyAverage_rmse <- RMSE(validation$rating, mu_hat)
onlyAverage_rmse
```

A RMSE of 1.061 is not very accurate. It means our 
predictions are about 1 star off (on a 0.5 to 5 star scale) 
on average from the actual rating users give in the 
validation set.

Let's put this model into a list and start off our 
list of attempts:

```{r Results 01 d, include=TRUE}
...
The letter "b" (short for bias) represents the coefficient estimates of our predictors.
In this model we'll only have one "bias" predictor, which is movie bias.

``` {r Results 02 b, include=TRUE}
movie_avgs <- edx %>% 
  group_by(movieId) %>% 
...
That's almost less than our goal of 0.8649! 
This is great! We're getting close!
